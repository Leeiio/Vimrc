...
"                 Encoding menu just like what you do in
"                 firefox/IE.
"               Options:
...
function! s:FencCreateMenu() "{{{1
    ...
    for i in s:Fenc8bit
        ...
        exec 'menu &Encodin&g.&8bit\ encodings.'.fenname.' :call FencMenuSel("'.fencla.'")<CR>'
    endfor
    for i in s:Fenc16bit
        ...
        let fenname=substitute(fenname,' ','\\ ','g')
        let fenname=substitute(fenname,'\.','_','g')
        exec 'menu &Encodin&g.&16bit\ encodings.'.fenname.' :call FencMenuSel("'.fencla.'")<CR>'
    endfor
    for i in s:FencUnicode
        ...
        let fenname=substitute(fenname,' ','\\ ','g')
        let fenname=substitute(fenname,'\.','_','g')
        exec 'menu &Encodin&g.&Unicode.'.fenname.' :call FencMenuSel("'.fencla.'")<CR>'
    endfor
    menu &Encodin&g.-sep-                  :
    menu &Encodin&g.&Auto\ Detect           :FencAutoDetect<CR>
    menu &Encodin&g.&Toggle\ Encoding\ list :FencView<CR>
    menu &Encodin&g.&Input\.\.\.            :call FencMenuSel(inputdialog(g:FencCustom))<CR>
endfunction
